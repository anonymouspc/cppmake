...
from cppmakelib.utility.filesystem import get_file_modify_time, iterate_dir, normal_path, path, replace_file_suffix
...
class Object:
    ...
    build_dynamic_file   : path
    build_executable_file: path
    modify_time          : time
    link_flags           : list[str]
...
@pre(1, normal_path)
def __init__(self: Object, file: path, from_code: Module | Source) -> None: # TODO: abort trying `resignature` in `decorator` here.
    self.file            = file
        ...
        async with scheduler.schedule():
            ...
            await compiler.async_share(
                ...
                lib_files   =recursive_collect(self,            next=lambda object : object.lib_objects,       collect=lambda object : object.file) +
                             recursive_collect(context.package, next=lambda package: package.require_packages, collect=lambda package: [file for file in iterate_dir(package.install_lib_dir)], flatten=True)
            )
        context.package.unit_status_cacher.set_shared(object=self, shared=True)
...
@once
async def async_is_shared(self: Object) -> bool:
    return context.package.unit_status_cacher.get_shared(object=self)
...
async def async_link(self: Object) -> Executable:
    if not await self.async_is_linked():
        ...
        context.package.unit_status_cacher.set_linked(object=self, linked=True)
...
@once
async def async_is_linked(self: Object) -> bool:
    return context.package.unit_status_cacher.get_linked(object=self)