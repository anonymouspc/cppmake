...
namespace cppmake
{
    ...
    config_t::config_t ( int argc, char** argv )
    {
        ...
        option_description.add_options()
            ...
            ("install-dir",   boost::program_options::value<std::filesystem::path>   (&this->install_dir)  ->default_value(system.visit([] (auto&& system) { return system.install_path; })))
            ("compiler-path", boost::program_options::value<resolvable_path>         (&this->compiler_path)->default_value(system.visit([] (auto&& system) { return system.compiler_path; })))
